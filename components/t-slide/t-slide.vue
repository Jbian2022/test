	...
	<view>
	 ...
	 {{items}}
	 	...
	 	　　<view class="t-touch-item" :style="{height:height}" :class=" item.isTouchMove?'touch-move-active':''" @touchstart="touchstart($event,item)" @touchmove="touchmove" :data-index="index">
	 		      <view class="t-slide-content" :style="{transform:item.isTouchMove?'translateX(0)':`translateX(${btnWidth*(btnArr.length)}upx)`,
				                                marginLeft:`-${btnWidth*(btnArr.length)}upx`}" @click="itemClick(item)">
												
				    <slot name="content"> </slot>
	 		      </view>
				   <view  class="t-slide-btn" v-for="(btn,num) in btnArr" :key="num" @touchstart="btnClick(btn.events,item)" 
				    :style="{width:btnWidth+'upx',transform:item.isTouchMove?'translateX(0)':`translateX(${btnWidth*(btnArr.length)}upx)`,
		...
		watch: {
			sendList: {
				handler: function(n, o) {
					console.log(n, '????')
				let data =	n.forEach(val=>{this.$set(val,'isTouchMove',false)})
					this.items = data
				},
				immediate: true
			}
