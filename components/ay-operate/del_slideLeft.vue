<template>
	<view>
		<view class="box-slideLeft" >
			...
			<view class="touch-item del-box-touch-slideLeft cf-shuCenter"  @click="delItem(item_show)" :style="item_show.delteStyle">
		...
		methods: {
			...
			touchM: function(e) {
				...
				if (e.touches.length == 1) {
					...
					if (disX == 0 || disX < 0) { //如果移动距离小于等于0，说明向右滑动，文本层位置不变
						...
						delteStyle = "right:" + disX + 'upx'
						if (disX >= delBtnWidth) {
							//控制手指移动距离最大值为删除按钮的宽度
							txtStyle = "left:-" + delBtnWidth + "upx";
						
							delteStyle = "right:" + 15 + 'upx'
						}
					}
					//获取手指触摸的是哪一项
					that.item_show.txtStyle = txtStyle;
					that.item_show.delteStyle = delteStyle;
			...
			touchE: function(e) {
				...
				if (e.changedTouches.length == 1) {
					...
					var txtStyle = disX > delBtnWidth / 2 ? "left:-" + delBtnWidth + "upx" : "left:0upx";
					//获取手指触摸的是哪一项
					that.item_show.txtStyle = txtStyle;
