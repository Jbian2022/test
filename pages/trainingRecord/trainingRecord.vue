<template>
  <view class="training-record">
    ...
    <view class="status_bar" :class="{ isFixedTop: isFixedTop }">
      <!-- 这里是状态栏 -->
    </view>
    <van-nav-bar
      :class="{ isFixedTop: isFixedTop }"
      :title="memberName"
      left-text="返回主页"
      left-arrow
    ...
    <view class="calendar-box">
      ...
      >
        ...
        <template #default="{ cell }">
          <view class="cell-box">
            <view class="cell-key" :class="{ active: cell.isSelected }">{{
              cell.key
            }}</view>
            <view v-if="trainListInfo[cell.day]" class="cell-label-box">
              <view
                class="cell-label"
                v-for="(item, key) in trainListInfo[cell.day]"
                :key="key"
                @click.stop="dataAndLabelHandle(cell)"
                >{{ item.traineeTitle }}</view
              >
            </view>
          </view>
        </template>
    ...
    <view class="footer-button">
      <view class="tipes" v-if="showTipes"> 开始今日训练吧 </view>
      ...
      <view class="action-box">
        ...
        <view class="content">
          <view
            ...
            :key="key"
            @click="traineeTitleHandle(currentDay, key, item)"
            ><view>{{ item.traineeTitle }}</view
            ><van-icon name="arrow"
          /></view>
        </view>
        <view class="label" v-if="!isButton">每日最多添加三次训练记录</view>
        <view class="action-button" v-else @click="selectHandle(currentDay)"
          >新建训练</view
        >
      </view>
    </uni-popup>
...
export default {
  ...
  methods: {
    ...
    dataAndLabelHandle(item) {
      ...
      if (
        (!this.trainListInfo[item.day] && item.disabled) ||
        item.type !== 'current'
      ) {
        return
      }
      ...
      this.isButton =
        (this.trainListInfo[item.day] &&
          this.trainListInfo[item.day].length < 3) ||
        !this.trainListInfo[item.day]
    ...
    selectHandle(day) {
      ...
      uni.navigateTo({
        url:
          ...
          `?traineeNo=${this.traineeNo}&trainDate=${day}&traineeName=${this.memberName}`
      })
    },
    traineeTitleHandle(date, key, item) {
      ...
      if (item.traineeStatus === 'save') {
        uni.navigateTo({
          url:
            '/pages/newWorkout/newWorkout' +
            `?traineeNo=${this.traineeNo}&trainDate=${date}&traineeName=${this.memberName}&key=${key}`
        })
        return
      }
      uni.navigateTo({
    ...
    addWorkout() {
      if (
        this.trainListInfo[this.getDay(new Date())] &&
        this.trainListInfo[this.getDay(new Date())].length >= 3
      ) {
      ...
      uni.navigateTo({
        url:
          '/pages/newWorkout/newWorkout' +
          `?traineeNo=${this.traineeNo}&trainDate=${this.getDay(
            new Date()
