  ...
  <view class="new-workout">
    ...
    <view class="header">
      ...
      <view>
        ...
        <van-button
          class="btn"
          v-if="isShowSave && isShowSuccess"
          @click="openDialog('popupFinish')"
          >完成训练</van-button
        >
      </view>
    </view>
      ...
      <view v-for="(i, ix) in actionList" :key="ix" class="action-type-box">
        <view v-if="i.type === 0" class="action-tiem" @click="changeOpen(i)">
          <view class="action-tiem-header">
            ...
            >
              ...
              <template v-slot:item="{ item }">
                <text
                  v-if="item.text === '删除动作项'"
                  style="color: #f04242"
                  @click="deleteActionHandle(ix)"
                  >{{ item.text }}</text
                >
          ...
          <view v-show="i.open" class="action-tiem-des">
            <view
              v-for="(item, index) in i.groupList"
              :key="index"
              class="project-item"
            ...
            >
              ...
              <view
                class="yes"
                @click=";(item.active = !item.active), technicalData()"
              >
            ...
            <view class="add-project-item" @click="addProjectItem(i.groupList)"
              ><text class="add-text">+</text> 新增一组</view
            >
          </view>
        </view>
        ...
        <view v-if="i.type === 2" class="action-tiem" @click="changeOpen(i)">
          ...
          <view v-show="i.open" class="action-tiem-des">
            ...
            <view
              ...
              @click="addProjectItem(i.groupList), technicalData()"
              ><text class="add-text">+</text> 新增一组</view
            >
          </view>
        </view>
    ...
    <view class="footer-button">
      ...
      <van-button block class="add" @click="addActionHandle"
        >+ 添加动作</van-button
      >
    </view>
    <uni-popup
    ...
    >
      <view class="delete-dialog">
        ...
        <view class="botton-box">
          <van-button class="delete" block @click="deleteHandle"
            >确认删除</van-button
          >
          <van-button block @click="closeDialog('popupDelete')"
            >取消</van-button
          >
        </view>
      </view>
...
export default {
  ...
  methods: {
    ...
    async finish(traineeStatus) {
      ...
      if (
        (this.allWork && this.allWork.length > 1) ||
        this.key ||
        this.isNoOldInfo
      ) {
        ...
        if (item.type === 0) {
          ...
          item.times = item.groupList.reduce(function (prev, cur) {
            if (!cur.hour || !cur.minute || !cur.second) {
              ...
              return (
                +cur.hour * 60 * 60 + +cur.minute * 60 + +cur.second + +prev
              )
            }
          }, 0)
        } else if (item.type === 2) {
          ...
          item.load = item.groupList.reduce(function (prev, cur) {
            if (!cur.kg || !cur.time) {
              ...
              if (item.weight) {
                return cur.active
                  ? (+cur.kg + +item.weight) * cur.time + +prev
                  : prev
              } else {
                return cur.active ? cur.kg * cur.time + +prev : prev
              }
