<template>
	<view class="counter">
		<view class="text">
			<text class="text1">健变，<br> 从此刻开始。</text><br>
			<text class="text2">私人专属教练，扁平学员管理</text>
		</view>
		<view class="middle">
			<van-field v-model="value"  readonly clickable
			 class="phone"
			placeholder="请输入手机号"
			...
			<van-number-keyboard
			  ...
			  @blur="show = false"
			/>
			<button class="btn" :class="controlActiveFlag ? 'active_btn' : ''"  @click.native="getSms"><span class="btn-text">获取验证码</span></button>
			<view class="ying_si_style">
				<view class="check_style" @click="checkFlag=!checkFlag">
					<image class="check_img_style" src="../../static/login/check.svg" v-if="checkFlag"></image>
					<image class="check_img_style" src="../../static/login/nocheck.svg" v-if="!checkFlag"></image>
				</view>
				...
				<view class="botter" v-if="false">
					<view class="botter-top">
						...
						<p class="botter-top3">为了更好的保障您的合法权益，在使用本应用之前，请您仔细阅读《隐私协议》和《用户协议》，点击同意即表示您已阅读并同意接受我们的服务，感谢您的信任！</p>
						<button class="botter-top4"><span class="botter-top4-text">同意并继续</span></button>
						<view class="botter-top5-text" @click.native="checkFlag=false">不同意</view>
			...
			<view class="wx_loging_style"  @click.native="loginByWeixin">
				<image  @click.native="loginByWeixin" class="wx_img_style" src="../../static/login/wxlogin.svg"></image>
			</view>
		</view>
