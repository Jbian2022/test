  ...
  <view class="action-library">
    ...
    <view class="header">
      <view
        class="all-action"
        :class="{ active: mode === 0 }"
        @click="modeChangeHandle(0)"
        >全部动作库</view
      >
      <view
        class="problem-action"
        :class="{ active: mode === 1 }"
        @click="modeChangeHandle(1)"
        >问题动作库</view
      >
      <view class="custom-action" @click="addActionHandle('z')"
        >+ 自定义动作</view
      >
      <!-- 线条 -->
    ...
    <view class="content" :class="{ 'select-page': showSaveButton }">
      ...
      <view class="action-list">
        ...
        <view class="action-list-view">
          <view
            v-for="(classify, classifyi) in actionList"
            :key="classifyi"
            class="classify-box"
          >
            <view class="classify">{{
              actionTypeList[classify.actionType].title
            }}</view>
              ...
              <template v-for="i in classify.children" :key="i._id">
                ...
                >
                  <popover
                    ...
                    :list="actions"
                    mode="longpress"
                    @selctClick="selectClick($event, i)"
                  >
    ...
    <view v-if="showSaveButton" class="footer-button">
      ...
      <van-button block type="primary" @click="goNewWorkout"
        >确认添加（{{ selectNum }}）</van-button
      >
    </view>
    <uni-popup
      ref="popup"
...
export default {
  ...
  methods: {
      ...
      if (item.text === '删除动作') {
        ...
        uni.reLaunch({
          url:
            ...
            `?type=${this.mode}&actionClass=${this.actionClass}&update=1&id=${info._id}&actionName=${info.actionName}&actionType=${info.actionType}`
        })
      }
    },
    addActionHandle(z) {
    ...
    async deleteHandle() {
      ...
      const res = await actionLibrary.deleteAction({
        id: this.currentAction._id
      })
  ...
  computed: {
    ...
    tryme() {
      let leftValue = 1.3
      if (this.mode === 1) {
        leftValue = this.mode * 6.125 + 3
      }
      return leftValue + 'rem'
    }
  }
}
</script>
